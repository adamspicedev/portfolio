...
Timing the blocking endpoint results in the following: ``` time curl --get http://localhost:3000/block
17.382 total ``` As expected, this is far too slow. Now, let's try to speed it up
...
The results speak for themselves: ``` time curl --get http://localhost:3000/block
2.037 total ```
